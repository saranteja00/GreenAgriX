# ─── GreenAgriX Government Agricultural Schemes & Subsidies Engine ────────
...
GOVERNMENT_SCHEMES_DB = [
    {
        ...
        "subsidy_percentage_small_marginal": 100, # 100% subsidy for Small/Marginal in TN (<5 acres)
        "subsidy_percentage_other": 75,
        "eligible_crops": ["Tomato", "Chilli", "Vegetables", "Cotton", "Maize", "Horticulture", "Banana", "Sugarcane"],
        "min_acreage": 0.5,
        "max_acreage": 12.5,
        "description_en": "Provides 100% subsidy for small/marginal farmers (up to 5 acres) and 75% for other farmers to install automated drip/sprinkler irrigation systems, saving 60% water and increasing crop yield by 35%.",
        "description_ta": "சிறு மற்றும் குறு விவசாயிகளுக்கு 100% முழு மானியத்திலும், பிற விவசாயிகளுக்கு 75% மானியத்திலும் சொட்டுநீர் மற்றும் தெளிப்பு பாசன அமைப்புகள் வழங்கப்படுகின்றன. இது 60% நீர் சேமிப்பு மற்றும் 35% கூடுதல் மகசூலைத் தருகிறது.",
        "required_documents_en": [
            "Aadhaar Card copy",
            "Land Patta / Chitta & FMB Sketch",
            "Bank Passbook (Aadhaar linked)",
            "Small / Marginal Farmer Certificate (VAO / Tahsildar)",
        ...
        "application_steps_en": [
            ...
            "4. Assistant Director of Horticulture (ADH) field inspection",
            "5. Direct Work Order issuance and installation"
        ],
    ...
    },
    {
        ...
        "subsidy_percentage_small_marginal": 70,
        "subsidy_percentage_other": 60,
        "eligible_crops": ["All Crops", "Tomato", "Wheat", "Paddy", "Cotton", "Maize"],
        "min_acreage": 1.0,
        "max_acreage": 25.0,
        "description_en": "Provides 70% financial subsidy for standalone solar pumps (3 HP to 7.5 HP) for replacement of diesel pumps and off-grid borewell energization with zero electricity bills.",
    ...
    },
    {
        ...
        "subsidy_percentage_small_marginal": 100,
        "subsidy_percentage_other": 100,
        "eligible_crops": ["All Crops", "Tomato", "Wheat", "Paddy", "Cotton", "Maize"],
        "min_acreage": 0.1,
        "max_acreage": 20.0,
        "description_en": "Provides guaranteed financial assistance of ₹6,000 per year in three equal 4-monthly installments of ₹2,000 directly into the farmer's Aadhaar-seeded bank account.",
    ...
    },
    {
        ...
        "subsidy_percentage_small_marginal": 90,
        "subsidy_percentage_other": 85,
        "eligible_crops": ["Paddy", "Wheat", "Tomato", "Cotton", "Maize", "Pulses", "Oilseeds"],
        "min_acreage": 0.2,
        "max_acreage": 50.0,
        "description_en": "Comprehensive risk insurance against unseasonal heavy rains, drought, pests, and post-harvest losses. Farmers pay only 1.5% - 2% premium; rest 98% premium is borne by Government.",
    ...
    },
    {
        ...
        "subsidy_percentage_small_marginal": 50,
        "subsidy_percentage_other": 40,
        "eligible_crops": ["All Crops", "Paddy", "Cotton", "Tomato", "Sugarcane"],
        "min_acreage": 1.0,
        "max_acreage": 30.0,
        "description_en": "Provides 50% subsidy on Power Tillers, Mini Tractors, Multi-Crop Thrashers, Brush Cutters, and Power Sprayers for small/marginal farmers.",
    ...
    },
    {
        ...
        "min_acreage": 0.1,
        "max_acreage": 50.0,
        "description_en": "Free laboratory soil testing across 12 chemical & physical parameters (N, P, K, S, Zn, Fe, Cu, Mn, Bo, pH, EC, OC) with personalized fertilizer dosage recommendations every 2 years.",
        ...
        "application_steps_en": [
            "1. Submit soil sample to local Block Soil Testing Laboratory (STL)",
...
class SchemesService:
    def get_all_schemes(self) -> List[dict]:
        return GOVERNMENT_SCHEMES_DB

    def match_eligible_schemes(
        self,
        crop: str = "Tomato",
        acreage: float = 3.5,
        farmer_category: str = "Small / Marginal", # "Small / Marginal" | "General"
        has_well_borewell: bool = True,
        category_filter: Optional[str] = None
    ) -> dict:
        """
        Dynamically matches eligible schemes for the farmer's registered farm.
        Calculates exact subsidy percentage, monetary aid, and match score.
        """
        matched = []
        total_potential_savings = 0

        for s in GOVERNMENT_SCHEMES_DB:
            # Check crop suitability
            crop_eligible = "All Crops" in s["eligible_crops"] or any(
                c.lower() == crop.lower() for c in s["eligible_crops"]
            )
            # Check acreage bounds
            acreage_eligible = s["min_acreage"] <= acreage <= s["max_acreage"]

            # Category filter
            if category_filter and category_filter != "All" and s["category"] != category_filter:
                continue

            if crop_eligible and acreage_eligible:
                is_small_marginal = acreage <= 5.0 or "Small" in farmer_category
                subsidy_percent = (
                    s["subsidy_percentage_small_marginal"]
                    if is_small_marginal
                    else s["subsidy_percentage_other"]
                )

                # Estimated aid calculation
                if "Acre" in s["name_en"] or s["id"] == "scheme_drip_micro_irrigation":
                    estimated_aid = round(s["max_subsidy_amount_inr"] * min(acreage, 5.0), 0)
                else:
                    estimated_aid = s["max_subsidy_amount_inr"]

                total_potential_savings += estimated_aid

                matched.append({
                    **s,
                    "is_eligible": True,
                    "match_score": 98 if is_small_marginal else 88,
                    "applicable_subsidy_percentage": subsidy_percent,
                    "calculated_financial_aid_inr": estimated_aid,
                    "farmer_category_applied": "Small/Marginal Farmer (சிறு/குறு விவசாயி)" if is_small_marginal else "General Farmer (பொது விவசாயி)",
                })
        ...
        return {
            "farmer_profile": {
                ...
                "has_water_source": has_well_borewell
            },
            "eligible_schemes_count": len(matched),
            "total_potential_subsidy_savings_inr": total_potential_savings,
